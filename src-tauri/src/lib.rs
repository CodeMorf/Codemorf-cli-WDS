...
use serde::Serialize;
use std::{fs, path::PathBuf, process::Stdio, sync::Mutex};
...
async fn run_command(command: String, cwd: Option<String>, shell: Option<String>) -> Result<CommandResult, String> {
    ...
    let mut cmd = if cfg!(target_os = "windows") {
        if shell == "cmd" {
            ...
            c.args(["-NoLogo", "-NoProfile", "-NonInteractive", "-ExecutionPolicy", "Bypass", "-Command", &command]);
            c
        }
    } else {
...
fn resolve_grok_binary() -> Option<PathBuf> {
    if let Ok(custom) = std::env::var("CODEMORF_GROK_CLI") {
        let path = PathBuf::from(custom);
        if path.exists() { return Some(path); }
    }
    if let Ok(path) = which::which("grok") { return Some(path); }
    if let Ok(home) = std::env::var("USERPROFILE") {
        let path = PathBuf::from(home).join(".grok").join("bin").join("grok.exe");
        if path.exists() { return Some(path); }
    }
    None
}
...
async fn grok_detect() -> GrokStatus {
    if let Some(path) = resolve_grok_binary() {
        ...
        GrokStatus { installed: false, path: None, version: None }
    }
}

#[tauri::command]
async fn grok_start(app: AppHandle, runtime: State<'_, GrokRuntime>, cwd: String, always_approve: bool) -> Result<(), String> {
    {
        let guard = runtime.process.lock().map_err(|_| "No se pudo bloquear el runtime Grok".to_string())?;
    ...
    command.arg("agent");
    if always_approve { command.arg("--always-approve"); }
    command.arg("stdio");
    ...
    tauri::async_runtime::spawn(async move {
        ...
        while let Ok(Some(line)) = lines.next_line().await {
            let _ = app_stdout.emit("grok://message", line);
        }
    });
    ...
    let mut guard = runtime.process.lock().map_err(|_| "No se pudo bloquear el runtime Grok".to_string())?;
    *guard = Some(GrokProcess { child, stdin });
    Ok(())
}

#[tauri::command]
async fn grok_send(runtime: State<'_, GrokRuntime>, payload: String) -> Result<(), String> {
    let mut proc = {
        let mut guard = runtime.process.lock().map_err(|_| "No se pudo bloquear el runtime Grok".to_string())?;
        guard.take().ok_or("Grok Build no está iniciado")?
    };
...
#[tauri::command]
async fn grok_stop(runtime: State<'_, GrokRuntime>) -> Result<(), String> {
    let proc = {
        let mut guard = runtime.process.lock().map_err(|_| "No se pudo bloquear el runtime Grok".to_string())?;
        guard.take()
    };
...
#[tauri::command]
fn memory_delete(app: AppHandle, id: i64) -> Result<(), String> {
    open_db(&app)?.execute("DELETE FROM memories WHERE id=?1", [id]).map_err(|e| e.to_string())?;
    Ok(())
}
...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![
            run_command, git_command, read_dir, read_text_file, write_text_file,
            grok_detect, grok_start, grok_send, grok_stop, open_external,
            memory_list, memory_upsert, memory_delete
        ])
        .run(tauri::generate_context!())
        .expect("error while running CodeMorf CLI");
}
